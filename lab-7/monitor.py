...
def main(waiting_time_interval, svc1, svc2, trainingfile, prometheus_port):
    ...
    gauge_mae = Gauge(f'lab7_{svc1}_2_{svc2}_MAE', 'Guage MAE')
    gauge_mape = Gauge(f'lab7_{svc1}_2_{svc2}_MAPE', 'Guage Mape')
    gauge_anomaly_counts = Gauge(f'lab7_{svc1}_2_{svc2}_anomaly_count', 'Anomaly Counts Gauge')
    y_max = Gauge(f'lab7_{svc1}_2_{svc2}_y_max', 'y maximum')
    y_min = Gauge(f'lab7_{svc1}_2_{svc2}_y_min', 'y minimum')
    y = Gauge(f'lab7_{svc1}_2_{svc2}_y_values', 'y values')
...
if __name__ == '__main__':
    try:
        ...
        print("Usage: python monitor.py -a <serviceA> -b <serviceB> -f <trainingfile> -p <port> <test_interval_minutes>")
        sys.exit(2)
    ...
    for opt, arg in opts:
        if opt in ("-a", "--serviceA"):
            ...
            prometheus_port = int(arg)

    if len(args) != 1:
        print("Usage: python monitor.py -a <serviceA> -b <serviceB> -f <trainingfile> -p <port> <test_interval_minutes>")
        sys.exit(2)

    waiting_time_interval = int(args[0])
    print("waiting time",waiting_time_interval)
    main(waiting_time_interval, svc1, svc2, trainingfile, prometheus_port)
